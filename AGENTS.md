...
**Pattern**: All async functions take `&Cx` as first parameter. The `Cx` flows down from the consumer's runtime — FrankenTorch does NOT create its own runtime.
...
| `ftui` (frankentui) | TUI rendering via frankentui |
| `serde` + `serde_json` | Serialization |
| `proptest` | Property-based testing (dev-dependency in ft-core) |
...
Cross-component integration tests live in the `ft-conformance` crate.
...
| Crate | Focus Areas |
|-------|-------------|
| `ft-core` | Tensor types, dtype conversions, shape arithmetic, stride computation, error types |
| `ft-dispatch` | Dispatcher routing, kernel registration, device-to-kernel mapping |
| `ft-kernel-cpu` | CPU kernel correctness, elementwise ops, reduction ops, broadcasting |
| `ft-autograd` | Gradient graph construction, backward pass correctness, alias/versioning rules |
| `ft-device` | Device enumeration, memory management, device selection |
| `ft-serialize` | Format round-trip, cross-framework compatibility |
...
## FrankenTorch — This Project

**This is the project you're working on.** FrankenTorch is a deterministic tensor computation and autograd engine implemented in Rust, designed as a ground-up reimplementation of core PyTorch semantics with provenance-complete gradient evidence.

### Crown-Jewel Innovation

**Deterministic Autograd Contract (DAC):** Replayable gradient graph execution with provenance-complete gradient evidence. Every gradient computation is deterministic, traceable, and auditable.

### Non-Regression Rule

**Autograd correctness outranks kernel speed.** No optimization may degrade gradient fidelity. This is the fundamental invariant of the project.
...
├── crates/
│   ├── ft-core/                   # Zero-dep core types: DType, Shape, Stride, TensorMeta, errors
│   ├── ft-dispatch/               # Dispatcher: routes ops to device-specific kernels
│   ├── ft-kernel-cpu/             # CPU kernel implementations (elementwise, reduction, etc.)
│   ├── ft-autograd/               # Autograd engine: gradient graph, backward pass, DAC
│   ├── ft-device/                 # Device abstraction and memory management
...
```toml
# ft-runtime feature flags
[features]
default = []
asupersync-integration = ["dep:asupersync"]    # Async runtime integration
frankentui-integration = ["dep:ftui"]          # TUI dashboard integration
```
...
Compatibility focus: Preserve PyTorch-observable tensor semantics, autograd contracts, and scoped optimizer behaviors.

### Security Doctrine

Security focus: Protect against gradient corruption, unsafe in-place mutation paths, and serialization mismatch or replay inconsistencies.
...
| Scenario | Tool | Why |
|----------|------|-----|
| "How does the autograd engine work?" | `warp_grep` | Exploratory; don't know where to start |
| "Where is the dispatcher implemented?" | `warp_grep` | Need to understand architecture |
| "Find all uses of `DType::Float32`" | `ripgrep` | Targeted literal search |
...
mcp__morph-mcp__warp_grep(
  repoPath: "/dp/frankentorch",
  query: "How does the deterministic autograd contract work?"
)
```
