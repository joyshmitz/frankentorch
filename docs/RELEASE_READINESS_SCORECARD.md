...
|---|---:|---:|---:|---|
| `frankentorch-kgs4.cod-b-scan-dim0` | cumsum/cumprod f64 dim0 fwd+bwd `[262144,64]` | mixed-location routing loss: cumsum `1.33x` slower, cumprod `1.77x` slower; candidate run moved workers and control regressed roughly 2x; RCH artifact retrieval ended with `RCH-E309` | no-ship; temporary no-zero cumprod backward fast path reverted | source unchanged; do not retry without current-origin same-worker proof and a PyTorch-capable comparator |
...
Measured-discipline score: `32/32` for the gauntlet lanes/features. Default
