...
fn main() {
    ...
    let x = s
        .tensor_variable(
            (0..4).map(|i| (i % 7) as f64 * 0.1 - 0.3).collect(),
            vec![1, 1, 2, 2],
            true,
        )
        .unwrap();
    ...
    let x = s
        .tensor_variable(
            (0..8).map(|i| (i % 7) as f64 * 0.1 - 0.3).collect(),
            vec![1, 2, 2, 2],
            true,
        )
        .unwrap();
    let w = s
        .tensor_variable(
            (0..36).map(|i| (i % 5) as f64 * 0.2 - 0.4).collect(),
            vec![2, 2, 3, 3],
            true,
        )
        .unwrap();
    ...
    let y = s
        .functional_conv_transpose2d(x, w, Some(b), (1, 1), (0, 0), (0, 0))
        .unwrap();
    ...
    println!(
        "ct gp: pen={pv:.6} gw_sum={:.6} gb=[{:.5}, {:.5}]",
        gw.iter().sum::<f64>(),
        gb[0],
        gb[1]
    );
