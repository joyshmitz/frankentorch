...
fn fill(batch: usize, n: usize) -> Vec<f64> {
    ...
    for plane in 0..batch {
        for r in 0..n {
            for c in 0..n {
                let v = ((((plane + 1) * (r + 2) * (c + 3)) % 19) as f64 - 9.0) * 0.05;
                a[plane * n * n + r * n + c] = v + if r == c { 3.0 + r as f64 } else { 0.0 };
            }
        }
    }
    a
...
fn run_ft(batch: usize, n: usize) -> Result<(f64, f64), Box<dyn Error>> {
    ...
    for _ in 0..5 {
        ...
        let loss = s.tensor_add(sl, su).map_err(boxed)?;
...
A=((((p+1)*(rows+2)*(cols+3))%19).to(torch.float64)-9.0)*0.05
A=A+torch.diag_embed(3.0+torch.arange(N,dtype=torch.float64).reshape(1,N))
def step():
    Ar=A.clone().requires_grad_(True)
    P,L,U=torch.linalg.lu(Ar)
    (L.sum()+U.sum()).backward()
    return Ar.grad
