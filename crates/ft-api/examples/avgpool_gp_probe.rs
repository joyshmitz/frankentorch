...
fn hd(name:&str, f32:bool, pad:(usize,usize), cip:bool){
    ...
    let r=(||{let y=s.functional_avg_pool2d(x,(2,2),(2,2),pad,false,cip)?;let sq=s.tensor_mul(y,y)?;let l=s.tensor_sum(sq)?;s.tensor_functional_hessian(l,x)})();
    match r{Ok(h)=>{let d:Vec<f64>=(0..16).map(|i|(h[i*16+i]*1e4).round()/1e4).collect();println!("{name}: {d:?}");},Err(e)=>println!("{name}: ERR {e:?}")}
}
fn main(){
