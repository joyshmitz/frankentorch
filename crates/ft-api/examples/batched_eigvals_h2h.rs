...
fn main() {
    for (bb,k) in [(100000usize,4usize),(20000,16)] {
        ...
        let mut best=f64::INFINITY; let mut resum=0.0;
        for _ in 0..5 {
            let mut s=FrankenTorchSession::new(ExecutionMode::Strict);
            let x=s.tensor_variable(data.clone(),vec![bb,k,k],false).unwrap();
            let t=Instant::now();
            let y=s.tensor_linalg_eigvals(x).unwrap();
            let el=t.elapsed().as_secs_f64()*1e3;
            if el<best { best=el; let v=s.tensor_values(y).unwrap(); resum=v.iter().step_by(2).sum(); }
        }
        let pysrc=format!(r#"
import time, torch
...
for _ in range(2): torch.linalg.eigvals(A)
ts=[]
for _ in range(5):
    t=time.perf_counter(); torch.linalg.eigvals(A); ts.append((time.perf_counter()-t)*1e3)
w=torch.linalg.eigvals(A); print("MS",sorted(ts)[0]); print("RESUM",w.real.double().sum().item())
"#);
        let python=std::env::var("PYTORCH_PYTHON").unwrap_or("python3".into());
        print!("B={bb} k={k}: FT {best:.1} ms  resum {resum:.5e}");
        ...
        if let (Some(p),Some(pr))=(g("MS "),g("RESUM ")) {
            let rel=(resum-pr).abs()/(pr.abs()+1e-9); let rr=p/best;
            println!("  | PyTorch {p:.1} ms  resum {pr:.5e} | {} | {}", if rel<1e-9 {"MATCH".to_string()} else {format!("rel{rel:.1e}")}, if rr>=1.0 {format!("FT {rr:.2}x FASTER")} else {format!("FT {:.2}x slower",1.0/rr)});
        }
    }
}
