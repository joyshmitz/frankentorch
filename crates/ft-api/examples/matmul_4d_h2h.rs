...
fn main() {
    for (bd,h,k) in [(10000usize,8usize,16usize),(10000,8,4),(2000,12,32)] {
        let n=bd*h*k*k;
        let a:Vec<f64>=(0..n).map(|x| (((x*2654435761usize)%9973) as f64)*0.001-5.0).collect();
        let b:Vec<f64>=(0..n).map(|x| (((x*40503usize)%9973) as f64)*0.001-5.0).collect();
        let mut best=f64::INFINITY; let mut chk=0.0;
        for _ in 0..6 {
            let mut s=FrankenTorchSession::new(ExecutionMode::Strict);
            let x=s.tensor_variable(a.clone(),vec![bd,h,k,k],false).unwrap();
            let y=s.tensor_variable(b.clone(),vec![bd,h,k,k],false).unwrap();
        ...
        let py=format!(r#"
import time, torch
torch.set_num_threads(8); B,H,k={bd},{h},{k}
A=(((torch.arange(B*H*k*k,dtype=torch.float64)*2654435761)%9973)*0.001-5.0).reshape(B,H,k,k)
Bm=(((torch.arange(B*H*k*k,dtype=torch.float64)*40503)%9973)*0.001-5.0).reshape(B,H,k,k)
...
"#);
        ...
        print!("[{bd},{h},{k},{k}]: FT {best:.1}ms chk {chk:.4e}");
