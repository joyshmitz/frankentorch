...
fn main(){
  for (b,k) in [(20000usize,16usize),(100000usize,4usize),(5000usize,32usize)]{
    let mut a=vec![0.0f64;b*k*k]; let mut rhs=vec![0.0f64;b*k*k];
    for m in 0..b{for i in 0..k{for j in 0..k{ a[m*k*k+i*k+j]=if i==j{(k as f64)+1.0}else{0.1/((i+j+1)as f64)}; rhs[m*k*k+i*k+j]=((i+j)as f64)*0.01+0.5; }}}
    let mut s=FrankenTorchSession::new(ExecutionMode::Strict);
    let av=s.tensor_variable(a.clone(),vec![b,k,k],false).unwrap();
    let bv=s.tensor_variable(rhs.clone(),vec![b,k,k],false).unwrap();
    let op=|s:&mut FrankenTorchSession|->f64{ let x=s.tensor_linalg_solve(av,bv).unwrap(); s.tensor_values(x).unwrap().iter().map(|v|v.abs()).sum() };
    ...
    let py=format!("import time,torch\ntorch.set_num_threads(32)\nB,K={b},{k}\nv=torch.zeros(B,K,K,dtype=torch.float64)\nfor i in range(K):\n v[:,i,i]=K+1.0\n for j in range(K):\n  if i!=j: v[:,i,j]=0.1/(i+j+1)\nr=torch.zeros(B,K,K,dtype=torch.float64)\nfor i in range(K):\n for j in range(K): r[:,i,j]=(i+j)*0.01+0.5\nfor _ in range(3): torch.linalg.solve(v,r).abs().sum().item()\nimport time as tt;s=tt.perf_counter()\nfor _ in range(20): cc=torch.linalg.solve(v,r).abs().sum().item()\nprint((tt.perf_counter()-s)/20*1e3, cc)");
    ...
    println!("solve [{b},{k},{k}]: FT {:.2}ms PyTorch {:.2}ms => {} (rel {:.1e})", t[0], pms, if r>=1.0{format!("FT {:.2}x FASTER",r)}else{format!("FT {:.2}x slower",1.0/r)}, rel);
  }
}
