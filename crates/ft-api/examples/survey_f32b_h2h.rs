...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let b: Vec<f32>=(0..R*C).map(|i|(i%13) as f32 - 6.0 + 0.3).collect();
...
b=((torch.arange(R*C,dtype=torch.int64)%13).float()-6.0+0.3).reshape(R,C)
def t(fn,n=7):
    for _ in range(2): fn()
    ts=[]
...
for name,fn in [("cat_anchor",lambda:torch.cat([a,a],1)),
                ("atan2",lambda:torch.atan2(a,b)),
                ("fmod",lambda:torch.fmod(a,b)),
                ("mse_none",lambda:Fn.mse_loss(a,b,reduction='none')),
                ("l1_none",lambda:Fn.l1_loss(a,b,reduction='none'))]:
...
"#;
    ...
    let rep=|n:&str,ft:f64|{ if let Some(p)=pt.lines().find_map(|l|{let mut it=l.strip_prefix("PT ")?.split_whitespace(); if it.next()?==n {it.next()?.parse::<f64>().ok()} else {None}}){let r=p/ft; let tag=if r>=1.0{format!("FT {r:.2}x FASTER")}else{format!("FT {:.2}x SLOWER",1.0/r)}; println!("  {n:<10} {ft:8.3} {p:8.3}   {tag}");}};
    println!("op          FT(ms)    PT(ms)   verdict");
    rep("cat_anchor", t2(&a,&b,|s,x,_|{let _=s.tensor_cat(&[x,x],1);}));
    rep("atan2", t2(&a,&b,|s,x,y|{let _=s.tensor_atan2(x,y);}));
    rep("fmod", t2(&a,&b,|s,x,y|{let _=s.tensor_fmod(x,y);}));
    rep("mse_none", t2(&a,&b,|s,x,y|{let _=s.tensor_mse_loss(x,y,"none");}));
    rep("l1_none", t2(&a,&b,|s,x,y|{let _=s.tensor_l1_loss(x,y,"none");}));
    Ok(())
}
