...
fn run(
    ...
    match (|| {
        ...
        Ok(h) => {
            ...
            println!("{name:<22} diag={d:?}{}", if allzero { " (ALL ZERO)" } else { "" });
        }
...
fn main() {
    ...
    for &f32 in &[false, true] {
        ...
        run(&format!("layer_norm{tag}"), (0..8).map(|i| (i % 5) as f64 * 0.3 - 0.5).collect(), vec![2, 4], f32,
            |s, x| s.functional_layer_norm(x, vec![4], None, None, 1e-5), 8);
        // rms_norm over last dim (4), input [2,4]
        run(&format!("rms_norm{tag}"), (0..8).map(|i| (i % 5) as f64 * 0.3 - 0.5).collect(), vec![2, 4], f32,
            |s, x| s.functional_rms_norm(x, vec![4], None, 1e-5), 8);
        // group_norm 2 groups, input [1,4,2,2]
        run(&format!("group_norm{tag}"), x16.clone(), vec![1, 4, 2, 2], f32,
            |s, x| s.functional_group_norm(x, 2, None, None, 1e-5), 8);
        // avg_pool2d 2x2 s2, input [1,1,4,4]
        run(&format!("avg_pool2d{tag}"), x16.clone(), vec![1, 1, 4, 4], f32,
            |s, x| s.functional_avg_pool2d(x, (2, 2), (2, 2), (0, 0), false, true), 8);
        // max_pool2d 2x2 s2, input [1,1,4,4]
        run(&format!("max_pool2d{tag}"), x16.clone(), vec![1, 1, 4, 4], f32,
            |s, x| s.functional_max_pool2d(x, (2, 2), (2, 2)), 8);
    }
}
