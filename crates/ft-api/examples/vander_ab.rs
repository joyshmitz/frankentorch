...
fn main() {
    ...
    let run = |s: &mut FrankenTorchSession| {
        let v = s.tensor_variable(x.clone(), vec![rows], false).unwrap();
        std::hint::black_box(s.tensor_vander(v, Some(cols), true).unwrap());
    };
    ...
    println!(
        "vander [{rows},{cols}]: OLD(1t) {old:.2}ms NEW({nthreads}t) {new:.2}ms => {:.2}x",
        old / new
    );
}
