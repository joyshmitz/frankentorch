...
fn run_ft(batch: usize, n: usize) -> Result<(f64, f64), Box<dyn Error>> {
    ...
    for _ in 0..5 {
        ...
        let (_q, r) = s.tensor_linalg_qr(a, false).map_err(boxed)?; // complete
        let loss = s.tensor_sum(r).map_err(boxed)?;
...
A=A+torch.eye(N,dtype=torch.float64).unsqueeze(0)*(3.0+rows.to(torch.float64))
def step():
    Ar=A.clone().requires_grad_(True)
    Q,R=torch.linalg.qr(Ar,mode="complete")
    R.sum().backward()
    return Ar.grad
