//! Unary special-function scan FT vs PyTorch ([4000,4000] f64 no-grad). Hunting the
//! apply_function unconditional-save + serial-compute pattern in single-input ops.
//! Input in [0.1,0.9] (safe for erfinv/digamma/lgamma). `cat` is a landed-win ANCHOR.
//! Run: PYTORCH_PYTHON=/path/to/python cargo run --release -p ft-api --example unary_special_h2h
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let a: Vec<f64> = (0..R * C).map(|i| 0.1 + ((i % 9) as f64) * 0.09).collect();
    type Op = fn(&mut FrankenTorchSession, ft_autograd::TensorNodeId);
    let ops: Vec<(&str, Op)> = vec![
        ("cat_anchor", |s, x| { let _ = s.tensor_cat(&[x, x], 1); }),
        ("erf", |s, x| { let _ = s.tensor_erf(x); }),
        ("erfc", |s, x| { let _ = s.tensor_erfc(x); }),
        ("erfinv", |s, x| { let _ = s.tensor_erfinv(x); }),
        ("digamma", |s, x| { let _ = s.tensor_digamma(x); }),
        ("lgamma", |s, x| { let _ = s.tensor_lgamma(x); }),
        ("sinc", |s, x| { let _ = s.tensor_sinc(x); }),
        ("i0", |s, x| { let _ = s.tensor_i0(x); }),
    ];
...
x=(0.1+(idx%9).double()*0.09).reshape(R,C)
def t(fn,n=7):
    for _ in range(2):
        try: fn()
...
for name,fn in [("cat_anchor",lambda:torch.cat([x,x],1)),
                ...
                ("erfc",lambda:torch.erfc(x)),
                ("erfinv",lambda:torch.erfinv(x)),
                ("digamma",lambda:torch.digamma(x)),
                ("lgamma",lambda:torch.lgamma(x)),
                ("sinc",lambda:torch.sinc(x)),
                ("i0",lambda:torch.special.i0(x))]:
