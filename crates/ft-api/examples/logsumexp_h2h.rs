...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tt = |w: u8| { let mut best=f64::INFINITY; for _ in 0..7 {
        ...
        let ti=Instant::now();
        match w {0=>{let _=s.tensor_add(x,x);}1=>{let _=s.tensor_logsumexp(x,1);}_=>{let _=s.tensor_logsumexp(x,0);}}
...
print("PT lse_dim1 %.3f"%tm(lambda:torch.logsumexp(mat,1)))
print("PT lse_dim0 %.3f"%tm(lambda:torch.logsumexp(mat,0)))
"#, m=m);
    ...
    println!("logsumexp [4096,4096] (torch 8t / FT default), min-of-7");
    for (lbl,w) in [("add",0u8),("lse_dim1",1),("lse_dim0",2)] { let ft=tt(w); println!("  {lbl:<9} FT {ft:8.3}  PT {:8.3}  => {}",g(lbl),v(ft,g(lbl))); }
    Ok(())
}
