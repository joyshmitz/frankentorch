//! rot90 FT vs PyTorch ([4000,4000] f64 no-grad). FT rot90 composes transpose+flip (k=1,3)
//! or two single-dim flips (k=2); torch.rot90 uses a transpose VIEW + one flip. Probes whether
//! FT's extra materialization(s) lose. `cat` is a landed-win ANCHOR.
//! Run: PYTORCH_PYTHON=/path/to/python cargo run --release -p ft-api --example rot90_h2h
...
for name,fn in [("cat_anchor",lambda:torch.cat([x,x],1)),
                ("rot90_k1",lambda:torch.rot90(x,1,[0,1])),
                ("rot90_k2",lambda:torch.rot90(x,2,[0,1])),
