...
fn main() {
    for (bb,k) in [(100000usize,4usize),(20000,16)] {
        ...
        for _ in 0..5 {
            ...
            let t=Instant::now();
            let (w,_v)=s.tensor_linalg_eig(x).unwrap();
            let el=t.elapsed().as_secs_f64()*1e3;
            if el<best { best=el; let vv=s.tensor_values(w).unwrap(); resum=vv.iter().step_by(2).sum(); }
        }
        let pysrc=format!(r#"
import time, torch
...
for _ in range(2): torch.linalg.eig(A)
ts=[]
for _ in range(5):
    t=time.perf_counter(); torch.linalg.eig(A); ts.append((time.perf_counter()-t)*1e3)
w,_=torch.linalg.eig(A); print("MS",sorted(ts)[0]); print("RESUM",w.real.double().sum().item())
"#);
