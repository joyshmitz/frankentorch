// index_add f32 vs torch (no-grad): direct bucket-parallel accumulate. cc.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let srcv: Vec<f32> = (0..nsrc * cols).map(|i| ((i % 7919) as f32) * 0.001 + 0.5).collect();
    let bench = || {
        ...
        for _ in 0..5 {
            ...
            let sr = s.tensor_variable_f32(srcv.clone(), vec![nsrc, cols], false).unwrap();
            let t = Instant::now();
            let _ = s.tensor_index_add(xi, 0, ix, sr);
    ...
    let ix = s.tensor_variable(idxv.clone(), vec![nsrc], false)?;
    let sr = s.tensor_variable_f32(srcv.clone(), vec![nsrc, cols], false)?;
    let o = s.tensor_index_add(xi, 0, ix, sr)?;
...
def tm(fn,reps=5):
    ...
    return min(ts)
print("PT ia %.3f"%tm(lambda:input.index_add(0,idx,src)))
o=input.index_add(0,idx,src); assert o.dtype==torch.float32
...
"#);
    ...
    let pt = out.lines().find_map(|l| { let mut it = l.strip_prefix("PT ")?.split_whitespace(); if it.next()? == "ia" { it.next()?.parse::<f64>().ok() } else { None } }).unwrap_or(f64::NAN);
    let ft = bench();
    ...
    let _ = DType::F32;
    println!("index_add f32 dim=0 [{rows}x{cols}] nsrc={nsrc}: FT {ft:8.3}ms torch {pt:8.3}ms => {vrb} | dtype={dt:?} exact={exact} close={close}/{}", fv.len().min(tv.len()));
    Ok(())
}
