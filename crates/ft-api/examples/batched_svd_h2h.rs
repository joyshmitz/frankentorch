...
fn main() {
    for (bb,k) in [(100000usize,4usize),(50000,8),(20000,16)] {
        ...
        for _ in 0..5 {
            ...
            let t=Instant::now();
            let (_u,sv,_vh)=s.tensor_linalg_svd(x,true).unwrap();
...
for _ in range(2): torch.linalg.svd(A)
ts=[]
for _ in range(5):
    t=time.perf_counter(); torch.linalg.svd(A); ts.append((time.perf_counter()-t)*1e3)
_,S,_=torch.linalg.svd(A); print("MS",sorted(ts)[0]); print("SSUM",S.sum().item())
"#);
        ...
        if let (Some(p),Some(ps))=(g("MS "),g("SSUM ")) {
            ...
            println!("  | PyTorch {p:.1} ms  ssum {ps:.5e} | {} | {}", if rel<1e-6 {"MATCH".to_string()} else {format!("rel{rel:.1e}")}, if rr>=1.0 {format!("FT {rr:.2}x FASTER")} else {format!("FT {:.2}x slower",1.0/rr)});
        }
    }
}
