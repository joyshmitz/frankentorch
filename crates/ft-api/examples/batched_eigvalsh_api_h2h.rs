...
fn main() {
    for (bb,k) in [(100000usize,4usize),(20000,16),(4000,32)] {
        ...
        for _ in 0..5 {
            ...
            let t=Instant::now();
            let ev=s.tensor_linalg_eigvalsh(x).unwrap();
...
idx=torch.arange(B*k*k); b=idx//(k*k); r=(idx//k)%k; c=idx%k
d=(((b*7+r*13+c*5)%97).double())*0.01
A=d.reshape(B,k,k); A=(A+A.transpose(-1,-2))*0.5; A=A+k*torch.eye(k,dtype=torch.float64)
for _ in range(2): torch.linalg.eigvalsh(A)
ts=[]
for _ in range(5):
    t=time.perf_counter(); torch.linalg.eigvalsh(A); ts.append((time.perf_counter()-t)*1e3)
w=torch.linalg.eigvalsh(A); print("MS",sorted(ts)[0]); print("ESUM",w.sum().item())
"#);
