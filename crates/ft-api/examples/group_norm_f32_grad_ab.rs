...
//! group_norm_backward_f32). Fresh session/iter (tape never frees — gmuml).
...
fn main() {
    ...
    eprintln!("group_norm f32 fwd+bwd [{n},{c},{h}x{w}] g{groups}: composed {bo:.2} ms / fused {bn:.2} ms / speedup {:.2}x", bo / bn);
}
