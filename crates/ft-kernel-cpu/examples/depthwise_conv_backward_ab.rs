...
fn per_group(
    ...
    for ci in 0..c {
        ...
        for ni in 0..n {
            dch[ni * outplane..(ni + 1) * outplane]
                .copy_from_slice(&dout[(ni * c + ci) * outplane..(ni * c + ci) * outplane + outplane]);
            pch[ni * inplane..(ni + 1) * inplane]
                .copy_from_slice(&padded[(ni * c + ci) * inplane..(ni * c + ci) * inplane + inplane]);
        }
...
fn main() {
    for &(n, c, ph, pw) in &[(16usize, 64usize, 58, 58), (8, 256, 30, 30)] {
        ...
        let oh = (ph - kh) / sh + 1;
        let ow = (pw - kw) / sw + 1;
        let padded: Vec<f64> = (0..n * c * ph * pw).map(|i| (i % 877) as f64 * 0.01).collect();
        ...
        let (dp_g, dw_g) = per_group(&dout, &padded, &weight, n, c, ph, pw, kh, kw, oh, ow, sh, sw);
        let bx = dp_d.iter().zip(dp_g.iter()).all(|(a, b)| (a - b).abs() < 1e-9)
            && dw_d.iter().zip(dw_g.iter()).all(|(a, b)| (a - b).abs() < 1e-9);
        ...
        for _ in 0..reps {
            let t = Instant::now();
            std::hint::black_box(per_group(&dout, &padded, &weight, n, c, ph, pw, kh, kw, oh, ow, sh, sw));
