...
fn main() {
    for (bb, k) in [(100000usize, 4usize), (20000, 16), (4000, 32)] {
        ...
        for b in 0..bb.min(50) {
            let r = ft_kernel_cpu::eigh_contiguous_f64(&data[b * k * k..(b + 1) * k * k], &kmeta)
                .unwrap();
            for i in 0..k {
                if bv[b * k + i].to_bits() != r.eigenvalues[i].to_bits() {
                    ok = false;
                }
            }
        }
        println!(
            "B={bb} k={k}: FT batched eigh {best:.1} ms  esum {esum:.3e}  bit-exact-vs-loop:{}",
