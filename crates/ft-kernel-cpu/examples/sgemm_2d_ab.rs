...
fn timeit<F: FnMut()>(mut f: F) -> f64 {
    ...
    for _ in 0..6 { let s = Instant::now(); for _ in 0..10 { f(); } best = best.min(s.elapsed().as_secs_f64()/10.0*1000.0); }
    best
}
fn main() {
    println!("threads = {}", rayon::current_num_threads());
    for &(m, k, n) in &[(512usize, 1024, 1024), (1024, 1024, 1024), (4096, 256, 1024), (2048, 2048, 2048)] {
        ...
        let lin = timeit(|| { let _ = ft_kernel_cpu::linear_tensor_f32(&a, &w, None, m, k, n); });
        ...
        let mm = timeit(|| { let _ = ft_kernel_cpu::matmul_tensor_contiguous_f32(&a, &bsq, &am, &bm).unwrap(); });
        println!("m{m:>4} k{k:>4} n{n:>4}: linear(bt) {:>5.0}GF {lin:.3}ms | matmul {:>5.0}GF {mm:.3}ms", g/(lin/1e3), g/(mm/1e3));
    }
}
