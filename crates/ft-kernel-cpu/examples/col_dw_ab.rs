...
fn main() {
    ...
    for &(m, k, n) in &[(256usize, 64, 16384), (128, 1024, 4096), (64, 256, 8192)] {
        ...
        for _ in 0..3 { let _ = ft_kernel_cpu::matmul_tensor_contiguous_f64(&a, &b, &am, &bm).unwrap(); }
        let mut best = f64::MAX;
        for _ in 0..6 { let s = Instant::now(); for _ in 0..10 { let _ = ft_kernel_cpu::matmul_tensor_contiguous_f64(&a, &b, &am, &bm).unwrap(); } best = best.min(s.elapsed().as_secs_f64()/10.0*1000.0); }
        println!("wide m{m} k{k} n{n}: {best:.3} ms  {:.0} GFLOP/s", 2.0*m as f64*k as f64*n as f64/1e9/(best/1e3));
    }
}
