...
fn per_group_im2col(
    ...
    for ci in 0..c {
        ...
        for ni in 0..n {
            ...
            chan[ni * ph * pw..(ni + 1) * ph * pw]
                .copy_from_slice(&padded[src..src + ph * pw]);
        }
...
fn main() {
    ...
    for &(n, c, ph, pw, kh, kw) in &[(16usize, 64usize, 58, 58, 3, 3), (8, 256, 30, 30, 3, 3)] {
        ...
        let oh = (ph - kh) / sh + 1;
        let ow = (pw - kw) / sw + 1;
        let padded: Vec<f64> = (0..n * c * ph * pw).map(|i| (i % 977) as f64 * 0.01).collect();
        ...
        eprintln!("  per-group-im2col {bo:.2} ms / direct {bn:.2} ms / speedup {:.2}x\n", bo / bn);
    }
}
