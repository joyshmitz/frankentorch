...
fn main() {
    ...
    let run = || { std::hint::black_box(outer_tensor_contiguous_f64(&a, &b, &am, &bm).unwrap()); };
    ...
    println!("outer [{m}]x[{n}] -> {}M: OLD(1t) {old:.2}ms NEW({nthreads}t) {new:.2}ms => {:.2}x", m*n/1_000_000, old/new);
}
