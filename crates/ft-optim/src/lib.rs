...
impl Adamax {
    ...
    /// Maximize the objective instead of minimizing it (default: false).
    /// Mirrors `torch.optim.Adamax(maximize=...)`: negates the gradient first.
    #[must_use]
    pub fn maximize(mut self, maximize: bool) -> Self {
        self.maximize = maximize;
        self
...
impl Adadelta {
    ...
    /// Maximize the objective instead of minimizing it (default: false).
    /// Mirrors `torch.optim.Adadelta(maximize=...)`: negates the gradient first.
    #[must_use]
    pub fn maximize(mut self, maximize: bool) -> Self {
        self.maximize = maximize;
        self
...
impl ASGD {
    ...
    /// Maximize the objective instead of minimizing it (default: false).
    /// Mirrors `torch.optim.ASGD(maximize=...)`: negates the gradient first.
    #[must_use]
    pub fn maximize(mut self, maximize: bool) -> Self {
        self.maximize = maximize;
        self
...
impl Rprop {
    ...
    /// Maximize the objective instead of minimizing it (default: false).
    /// Mirrors `torch.optim.Rprop(maximize=...)`: negates the gradient first, so
    /// the sign-based step logic ascends the objective.
    #[must_use]
    pub fn maximize(mut self, maximize: bool) -> Self {
        self.maximize = maximize;
        self
...
mod tests {
    ...
    fn constant_lr_schedule_golden_matches_torch() {
        ...
        // (base*factor while epoch<total_iters, else base). ConstantLR has no
        // last_epoch() builder, so driven via the step(opt,None)+get_lr protocol
        // used by the existing step_lr tests. frankentorch-gixor.
        let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
        let x = s.tensor_variable(vec![1.0], vec![1], true).unwrap();
        let mut opt = SGD::new(vec![x], 1.0);
        let mut sch = ConstantLR::new(&opt, 0.5, 3);
        let want = [0.5, 0.5, 0.5, 1.0, 1.0];
        for (epoch, w) in want.iter().enumerate() {
            sch.step(&mut opt, None);
            let lr = opt.get_lr();
    ...
    fn cosine_annealing_lr_schedule_golden_matches_torch() {
        ...
        // (eta_min+(base-eta_min)*(1+cos(pi*e/T_max))/2). frankentorch-u7ja4.
        let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
        let x = s.tensor_variable(vec![1.0], vec![1], true).unwrap();
        let opt = SGD::new(vec![x], 1.0);
    ...
    #[test]
    fn exponential_lr_schedule_golden_matches_torch() {
        // Differential golden vs torch.optim.lr_scheduler.ExponentialLR 2.12:
        // base_lr=1, gamma=0.9 -> [1,0.9,0.81,0.729,0.6561] over epochs 0..4
        // (lr=base*gamma^epoch). frankentorch-9rujj.
        let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
        let x = s.tensor_variable(vec![1.0], vec![1], true).unwrap();
        let opt = SGD::new(vec![x], 1.0);
        let want = [1.0, 0.9, 0.81, 0.729, 0.6561];
        for (e, w) in want.iter().enumerate() {
    ...
    fn rmsprop_first_step_golden_matches_torch() {
        ...
        // (update=lr*g/(sqrt((1-alpha)*g^2)+eps)). frankentorch-effbj.
        let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
        let x = s.tensor_variable(vec![2.0, -3.0], vec![2], true).unwrap();
        let sq = s.tensor_mul(x, x).unwrap();
        let loss = s.tensor_sum(sq).unwrap();
        let report = s.tensor_backward(loss).unwrap();
        let mut opt = RMSprop::new(vec![x], 0.1);
        opt.step(&mut s, &report).unwrap();
        let after = s.tensor_values(x).unwrap();
        let want = [1.000_000_025, -2.000_000_016_667];
        for (g, w) in after.iter().zip(want.iter()) {
            assert!((g - w).abs() < 1e-7, "rmsprop step {g} != {w}");
        }
    }

    #[test]
    fn adamw_first_step_golden_matches_torch() {
        // Differential golden vs torch.optim.AdamW 2.12: x=[2,-3], lr=0.1, default
        // wd=0.01, loss=sum(x^2); one step == [1.89800000025,-2.897000000167]
        // (Adam update + decoupled -lr*wd*param). frankentorch-3rvg0.
        let mut s = FrankenTorchSession::new(ExecutionMode::Strict);
        let x = s.tensor_variable(vec![2.0, -3.0], vec![2], true).unwrap();
        let sq = s.tensor_mul(x, x).unwrap();
        let loss = s.tensor_sum(sq).unwrap();
        ...
        opt.step(&mut s, &report).unwrap();
        let after = s.tensor_values(x).unwrap();
        let want = [1.898_000_000_25, -2.897_000_000_167];
        for (g, w) in after.iter().zip(want.iter()) {
            assert!((g - w).abs() < 1e-9, "adamw step {g} != {w}");
        }
    }
