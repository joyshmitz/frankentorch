...
mod tests {
    ...
    fn dense_matrix_view_preserves_storage_offset() {
        ...
        assert_eq!(view.storage_id(), tensor.storage_id());
        assert_eq!(view.meta().storage_offset(), 2);
        assert_eq!(view.meta().shape(), &[4, 1]);
        assert_eq!(view.contiguous_values().unwrap(), &[1.0, 2.0, 3.0, 4.0]);
    }
    ...
    fn dense_scalar_view_preserves_storage_offset() {
        ...
        let view = tensor.view(vec![]).unwrap();

        assert_eq!(view.storage_id(), tensor.storage_id());
        assert_eq!(view.meta().storage_offset(), 1);
        assert_eq!(view.meta().shape(), &[] as &[usize]);
        assert_eq!(view.contiguous_values().unwrap(), &[7.0]);
    }
    ...
    #[test]
    fn sparse_coo_from_coords_nonempty_preserves_requested_f32_dtype() {
        let coords = vec![vec![0, 1], vec![1, 0]];
        let sparse = SparseCOOTensor::from_coords(
            &coords,
            vec![1.5, 2.5],
        ...
        match sparse.values().typed_storage() {
            TensorStorage::F32(values) => assert_eq!(values.as_slice(), &[1.5f32, 2.5]),
            other => panic!("expected F32 values storage, got {other:?}"),
        }
    ...
    #[test]
    fn sparse_coo_from_coords_nonempty_preserves_requested_f16_dtype() {
        let coords = vec![vec![0, 1], vec![1, 0]];
        let sparse = SparseCOOTensor::from_coords(
            &coords,
            vec![1.5, 2.5],
        ...
        match sparse.values().typed_storage() {
            ...
            other => panic!("expected F16 values storage, got {other:?}"),
        }

        let dense = sparse.to_dense().unwrap();
        assert_eq!(dense.meta().dtype(), DType::F16);
        assert_eq!(
            dense.contiguous_values_as_f64().unwrap(),
            &[0.0, 1.5, 2.5, 0.0]
        );
    ...
    #[test]
    fn sparse_coo_from_coords_nonempty_preserves_requested_complex64_dtype() {
        let coords = vec![vec![0, 1], vec![1, 0]];
        let sparse = SparseCOOTensor::from_coords(
            &coords,
            vec![1.5, 2.5],
        ...
        match sparse.values().typed_storage() {
            ...
            other => panic!("expected Complex64 values storage, got {other:?}"),
        }
    ...
    #[test]
    fn sparse_coo_empty_preserves_requested_bf16_dtype() {
        let sparse =
            SparseCOOTensor::from_coords(&[], vec![], vec![2, 2], DType::BF16, Device::Cpu)
                .unwrap();

        assert_eq!(sparse.dtype(), DType::BF16);
        assert!(matches!(
            sparse.values().typed_storage(),
            TensorStorage::BF16(values) if values.is_empty()
        ));
    ...
    fn sparse_coo_empty_preserves_requested_f16_dtype() {
        ...
        assert!(matches!(
            sparse.values().typed_storage(),
            TensorStorage::F16(values) if values.is_empty()
        ));
    ...
    fn sparse_coo_empty_preserves_requested_complex128_dtype() {
        ...
        assert!(matches!(
            sparse.values().typed_storage(),
            TensorStorage::Complex128(values) if values.is_empty()
        ));
