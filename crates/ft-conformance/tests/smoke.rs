...
#[test]
fn tensor_fft_fixture_executes_in_both_modes() {
    let cfg = HarnessConfig::default_paths();
    let (strict_report, strict_cases) = run_tensor_fft_conformance(&cfg, ExecutionMode::Strict)
        .expect("strict tensor-fft should run");
    let (hardened_report, _) = run_tensor_fft_conformance(&cfg, ExecutionMode::Hardened)
        .expect("hardened tensor-fft should run");

    assert!(
        strict_report.cases_total >= 10,
        "expected at least 10 FFT cases"
    );
    assert_eq!(strict_report.cases_total, strict_cases.len());
    assert_eq!(strict_report.cases_total, strict_report.cases_passed);
    assert_eq!(hardened_report.cases_total, hardened_report.cases_passed);
...
#[test]
fn tensor_pool_fixture_executes_in_both_modes() {
    let cfg = HarnessConfig::default_paths();
    let (strict_report, strict_cases) = run_tensor_pool_conformance(&cfg, ExecutionMode::Strict)
        .expect("strict tensor-pool should run");
    let (hardened_report, _) = run_tensor_pool_conformance(&cfg, ExecutionMode::Hardened)
        .expect("hardened tensor-pool should run");
...
#[test]
fn tensor_init_fixture_executes_in_both_modes() {
    let cfg = HarnessConfig::default_paths();
    let (strict_report, strict_cases) = run_tensor_init_conformance(&cfg, ExecutionMode::Strict)
        .expect("strict tensor-init should run");
    let (hardened_report, _) = run_tensor_init_conformance(&cfg, ExecutionMode::Hardened)
        .expect("hardened tensor-init should run");
